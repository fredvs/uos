...
uses
  ...
  cthreads,
  {$ENDIF}{$ENDIF}
   cwstring, SysUtils,uos, ctypes, Math,  Classes,fpg_button, fpg_edit, fpg_widget, fpg_label, fpg_Editbtn, fpg_RadioButton, fpg_trackbar, fpg_CheckBox, fpg_base, fpg_main, fpg_form
...
begin
  ...
  In1Index := Player1.AddFromFile(FilenameEdit4.filename, -1, 0);  //// add input from audio file with custom parameters
 ...
 FTIndex1 := Player1.AddFilterIn(In1Index, strtoint(edit2.text),strtoint(edit1.text), 1, typfilt, True, nil);
...
begin
 ...
 if radiobutton4.Checked = true then typfilt := 5;
 if assigned(Player1) and (btnstart.Enabled = false) then
  Player1.SetFilterIn(In1Index, FTIndex1,  strtoint(edit2.text),strtoint(edit1.text), 1, typfilt, True, checkbox2.Checked, nil);
...
begin
 ...
 if (200-TrackBar3.Position) > 100 then gain :=  1 + ((200- TrackBar3.Position) / 25)
...
begin
 ...
 if (200-TrackBar2.Position) > 100 then gain :=  1 + ((200- TrackBar2.Position) / 25)
...
var
  ...
  begin
   ...
   if (200-TrackBar1.Position) > 100 then gain :=  1 + ((200-TrackBar1.Position) / 25)
