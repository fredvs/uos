...
{$endif}  
   ...
   // function uos_loadlib(PortAudioFileName, SndFileFileName, Mpg123FileName, Mp4ffFileName, FaadFileName,  opusfilefilename, libxmpfilename: PChar) : LongInt;
    ...
    if uos_LoadLib(PChar(FilenameEdit1.FileName), nil, PChar(FilenameEdit3.FileName),
                         nil, nil, PChar(FilenameEdit2.FileName), nil) = 0 then
    begin
      hide;
      loadok           := True;
      Height           := 403;
    ...
    if loadok = True then
    begin
      if ((trim(PChar(filenameedit5.FileName)) <> '') and fileexists(filenameedit5.FileName)) and (uos_LoadPlugin('soundtouch', PChar(FilenameEdit5.FileName)) = 0) then
      begin
        plugsoundtouch := True;
        btnLoad.Text   :=
 ...
 {$ENDIF}
  ...
  {$IFDEF Darwin}
   ...
   {$ENDIF}
    ...
    FilenameEdit5.FileName := ordir + 'lib/Linux/arm_raspberrypi/plugin/libsoundtouch_arm.so';
      {$ENDIF}
