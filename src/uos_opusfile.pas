...
uses
  classes,
  {$IFNDEF FPC}
  DELPHIctypes,  pipesDelphi, windows,
  {$else}
  ctypes, dynlibs, pipes,
  {$endif}
  SysUtils;
...
begin
   ...
   if of_Handle<>0 then begin
      ...
      if Length(libfilename) = 0 then begin
         ...
         {$else}
         op_Handle:= SafeLoadLibrary('libopus.so');
         {$endif}
         of_Handle:=SafeLoadLibrary(libop);
   end else begin
      {$IFDEF windows}
      ...
      {$else}
      op_Handle:= SafeLoadLibrary(ExtractFilePath(libfilename)+'libopus.so');
      {$endif}
      of_Handle:=SafeLoadLibrary(libfilename);
   end;

   if of_Handle <> NilHandle then begin {now we tie the functions to the VARs from above}
      {$IFNDEF FPC}
